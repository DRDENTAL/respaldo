...
//leccion 8
	...
	//carga de colegios al sistema
	public function form_cargar_datos_colegios(){
       return view("formularios.form_cargar_datos_colegios");
	}


    public function cargar_datos_colegios(Request $request)
	{
       $archivo = $request->file('archivo');
       $nombre_original=$archivo->getClientOriginalName();
	   $extension=$archivo->getClientOriginalExtension();
       ...
       if($r1){
            ...
            Excel::selectSheetsByIndex(0)->load($ruta, function($hoja) {
		        ...
		        $hoja->each(function($fila) {
			        $colegiosemails=Colegios::where("email_estab","=",$fila->email_estab)->first();
			        if(count( $colegiosemails)==0){
				      	$colegios=new Colegios;
				        $colegios->cod_region= $fila->cod_region;
				        $colegios->des_comuna= $fila->des_comuna;
				        $colegios->rbd= $fila->rbd;
				        $colegios->dependencia= $fila->dependencia; //este campo llamado telefono se debe agregar en la base de datos c
				        $colegios->nombre_esta= $fila->nombre_esta;
				        $colegios->nombre_sostenedor= $fila->nombre_sostenedor;
				        $colegios->direccion_estab= $fila->direccion_estab;
		                $colegios->direccion_num= $fila->direccion_num;
		                //aca=== ($fila->direccion_num==null)?"":$fila->direccion_num;
		                $colegios->fono_estab= $fila->fono_estab;
		                $colegios->email_estab= $fila->email_estab;
		                $colegios->parvulos= $fila->parvulos;
		                $colegios->prim_a_cuar_basico= $fila->prim_a_cuar_basico;
		                $colegios->quin_a_sex_basico= $fila->quin_a_sex_basico;
		                $colegios->sep_a_oct_basico= $fila->sep_a_oct_basico;
		                $colegios->educ_media= $fila->educ_media;
		                $colegios->t_p_agric_marit= $fila->t_p_agric_marit;
		                $colegios->t_p_industrial= $fila->t_p_industrial;
		                $colegios->t_p_comercial_tecnico= $fila->t_p_comercial_tecnico;
		                $colegios->basica_adultos= $fila->basica_adultos;
		                $colegios->media_adultos= $fila->media_adultos;
		                $colegios->nt1= $fila->nt1;
		                $colegios->nt2= $fila->nt2;
